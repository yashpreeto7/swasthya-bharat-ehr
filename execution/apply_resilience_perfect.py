"""
execution/apply_resilience_perfect.py
Clean, deterministic script to hoist datasets to module scope and inject
rock-solid demo patient fallbacks, AI synthesis fallbacks, and connection monitoring.
"""
...
INITIAL_DEMO_PATIENTS_CODE = '''// ── ROCK-SOLID INITIAL DEMO PATIENT PROFILES (ALWAYS AVAILABLE) ──
...
def run():
    ...
    # 1. Locate the exact demo dataset block inside Home()
    start_marker = "  // ── RICH DEMO LAB REPORTS DATASET (NABL ISO 15189 ACCREDITED) ──"
    ...
    demo_block = content[start_idx:end_idx].rstrip()

    # Remove demo block from inside Home()
    content = content[:start_idx] + "\n\n" + content[end_idx:]

    # Place INITIAL_DEMO_PATIENTS and demo_block before export default function Home()
    home_decl = "export default function Home() {"
    replacement_top = f"{INITIAL_DEMO_PATIENTS_CODE}\n\n{demo_block}\n\n{home_decl}"
    content = content.replace(home_decl, replacement_top, 1)

    # 2. State initialization with initial demo patients and connection state
    old_state = """  // ── Doctor Workspace State ──
  const [patients, setPatients] = useState<any[]>([]);
  const [selectedPatientId, setSelectedPatientId] = useState<string>("");
  const [timeline, setTimeline] = useState<any>(null);"""

    new_state = """  // ── Doctor Workspace State ──
  ...
  const [timeline, setTimeline] = useState<any>(null);"""
    ...
    # 3. Selected patient finder and filtered events with fallback
    old_sel_pat = """  const selectedPatient = patients.find(p => p.patient_id === selectedPatientId);
  ...
  const filteredEvents = timeline?.events?.filter((evt: any) => {
    ...
    new_sel_pat = """  const selectedPatient = patients.find(p => p.patient_id === selectedPatientId || p.id === selectedPatientId || p.abha_id === selectedPatientId) || patients[0] || INITIAL_DEMO_PATIENTS[0];
  ...
  const filteredEvents = rawTimelineEvents.filter((evt: any) => {
    ...
    content = content.replace(old_sel_pat, new_sel_pat, 1)

    # 4. Auto-reconnect interval in useEffect
    ...
    content = content.replace(old_gen_ai, new_gen_ai, 1)

    # 7. Resilient askAiAssistant
    ...
    content = content.replace(old_ask_ai, new_ask_ai, 1)

    # 8. Patient Switcher in doctor_ai header
    ...
    content = content.replace(old_ai_header, new_ai_header, 1)

    # 9. Offline Simulation Banner above main
    ...
    content = content.replace(old_main_top, new_main_top, 1)

    # 10. Modernize footer gateway indicator
    old_footer_indicator = '<span className="text-emerald-400 font-semibold">● Gateway Connected</span>'
    ...
    content = content.replace(old_footer_indicator, new_footer_indicator, 1)

    PAGE_PATH.write_text(content, encoding="utf-8")
    print("Successfully applied perfect resilience to frontend/src/app/page.tsx!")
